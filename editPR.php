...
<div class="page-header">
	<h1>
		...
		<small>
			...
			Purchase Requisition : <?php echo $_GET['Order']; ?>
		</small>
	</h1>
</div><!-- /.page-header -->
	...
	<div class="col-xs-12">
		<div class="row">
			...
			<table width="80%" style="margin: : 10px">
					...
					<tbody>
						<td><input class="easyui-textbox" id="project" name="project" value="<?php echo $_GET['project']; ?>" style="width:500px;color: black;font-size: 15px;" placeholder="Optional" data-options="prompt:'Optional'">
						</td>
						<td >
							<select id="wrhs" class="easyui-dropdown" style="width: 200px" >
							</select>
						</td>
						<td><input readonly="true" id="dcdate" name="dcdate" value="<?php echo $_GET['dcdate']; ?>" style="width:150px" class="easyui-datebox">
						</td>
						<td><input id="rqdate" name="rqdate" value="" style="width:150px" class="easyui-datebox">
						</td>
...
<!-- Form Add Item -->
			...
			<tr>
				...
				<tr>
					...
					<td><input id="qty" name="qty" style="width:200px" class="easyui-numberbox" required="true" data-options="min:0,precision:0,decimalSeparator:'.',groupSeparator:','"></td>
					<td><h5>Total </td><td style="width: 20px">:</td>
					<td><input id="total" name="total" style="width:200px" class="easyui-numberbox" value="0" readonly="true" data-options="min:0,precision:3,decimalSeparator:'.',groupSeparator:','"></td>
				</tr>
				...
				<tr>
					...
					<td><input id="price" name="price" style="width:200px" class="easyui-numberbox" value="0" data-options="min:0,precision:3,decimalSeparator:'.',groupSeparator:','"></td>
					<td><h5>SPQ </td><td>:</td>
					<td>
...
<script type="text/javascript">		
	...
	function deleterow(){
		...
		$.messager.confirm('Confirm','Are you sure to delete PR item '+ index +' ?',function(r){
			if (r){
				...
				var deletePR = {
					docNum : ("<?php echo $_GET['Order'] ?>"),
					docItem : row.docItem,
				};
				$.ajax({
					type:'POST',
					url:'action/deleteItemPR.php',
					dataType: "json",
					data:deletePR,
					cache:false,
	...
	function showData(){
		var loadInd = "";
		if(loadInd == ""){
			var prNum = ("<?php echo $_GET['Order'] ?>");
			var url = 'action/getDetailPR.php?DocNum='+prNum;	
			$('#dg').datagrid({
				url:url
			})
			loadInd = "X";
	...
	function generateJSON(){
		...
		document.getElementById("smsg").innerHTML = "<center><br><h3>Order " + prNumb + " Updated</h3></center>";
		$('#smsg').dialog({
			title: 'Information',
			width: 400,
			height: 200,
	...
	function NewItem(param){		
		if(param == "new"){
			...
			if(rIndex == "-1"){
				...
				getCurrency(row.itemcode);
					$('#itemcode').textbox({
						iconAlign:'left',
						value:row.itemcode
					})

					if(row.itmname == "." || row.itmname == "-"){
						$('#itmname').textbox({
						iconAlign:'left',
						readonly:false,
						value:row.itmname
	...
	var dataTgl = ('<?php echo $_GET['rqdata']; ?>');
	var crDate = ('<?php echo $_GET['dcdate']; ?>');
	var indData = "";
	$(document).ready(function () { 
		...
		$('#dcdate').datebox('setValue', opts.formatter(new Date()));
			...
			$('document').ready(function() { 
			    $('#rqdate').datebox('setValue', crDate);
			});
		}	
	});	
	...
	$(document).ready(function () {	
		...
		$('#rqdate').datebox('setValue', opts.formatter(new Date()));
			...
			$('document').ready(function() { 
			    $('#rqdate').datebox('setValue', dataTgl);
			});
		}
	});	
